...
alter table public.email_action_requests
  add column if not exists response_protocol_version smallint not null default 1;

comment on column public.email_action_requests.response_token_hash is
  'SHA-256 hash of the single-use response token. The plaintext token exists only in Edge Function memory and the delivered email.';

comment on column public.email_action_requests.response_protocol_version is
  'Response authorisation protocol: version 1 validates the sender; version 2 requires a single-use response token.';
