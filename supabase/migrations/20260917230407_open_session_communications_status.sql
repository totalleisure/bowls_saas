-- Production promotion of the tested open-session communications fix.
-- Preserve existing function privileges and all non-open workflows.
BEGIN;
...
begin
  ...
  -- Open sessions have no selected squad or publication lifecycle.
  if v_selection_mode = 'open' and v_uses_rinks then
    return query select
      'open'::text, 'none'::text,
      'Players turn up to join this session. Team selection, publication and team-selection communications are not required.'::text,
      100, false, false, false, false, '[]'::jsonb,
      jsonb_build_object('selection_mode', v_selection_mode,
                        'is_internal', v_is_internal,
                        'communications_applicable', false);
    return;
  end if;
...
begin
  ...
  if v_base.status = 'open' or v_base.next_action = 'open_fixture' then
