...
class _CreateFixturePageState extends State<CreateFixturePage> {
  ...
  Future<void> _loadVenues() async {
    final homeVenues = await _client
        .from('venues')
        .select('id, name, town_city, postcode, is_home_venue')
        .eq('club_id', widget.clubId)
  ...
  Future<String?> _createVenueFromFixture({
    ...
    // Home venues are deliberately restricted to global superusers.
    ...
    // External/opponent venues can be created by authorised club officers.
                ...
                children: [
                  TextFormField(
                    ...
                    validator: (value) => value == null || value.trim().isEmpty
                        ...
                        : null,
                    onChanged: (value) => venueName = value,
                  ),
                  const SizedBox(height: 12),
                  TextFormField(
                    textCapitalization: TextCapitalization.words,
                    ...
                    onChanged: (value) => townCity = value,
                  ),
                  const SizedBox(height: 12),
                  TextFormField(
                  ...
                  Align(
                    ...
                    child: Text(
                      isHomeVenue
                          ...
                          : 'This will be saved as an opponent or external event venue.',
    ...
    try {
      final result = await _client.rpc(
        ...
        params: {
          ...
          // The user has already seen and accepted the duplicate warning in
          // this screen. The RPC still performs all permission, trimming and
          // normalisation checks server-side.
  ...
  Future<void> _testGoogleVenueSearch() async {
    if (_loading) return;
    ...
    try {
      final response = await _client.functions.invoke(
        ...
        body: {
          ...
          'query': 'Blackheath bowls club',
        },
      );

      debugPrint(
        'VENUE PLACES TEST status=${response.status} data=${response.data}',
      );

      if (!mounted) return;

      final prettyResult = const JsonEncoder.withIndent(
        '  ',
      ...
      await showDialog<void>(
        context: context,
        builder: (dialogContext) => AlertDialog(
          title: const Text('Google venue search test'),
          content: SizedBox(
            width: 620,
            child: SingleChildScrollView(
      ...
      debugPrint(
        'VENUE PLACES TEST function error '
      ...
      await _showSaveErrorDialog(
        'Google venue search test failed.\n\n'
        'HTTP ${e.status}\n'
        '${e.details ?? e.reasonPhrase ?? e.toString()}',
      );
    } catch (e, stackTrace) {
      debugPrint('VENUE PLACES TEST unexpected error: $e');
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [
            if (_isSuperuser || _isClubAdmin || _isSelector)
              IconButton(
                tooltip: 'Test Google venue search',
                onPressed: _loading ? null : _testGoogleVenueSearch,
