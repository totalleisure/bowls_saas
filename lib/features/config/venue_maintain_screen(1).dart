...
class _VenueMaintainScreenState extends State<VenueMaintainScreen> {
  ...
  Future<void> _findAndUpdateFromGoogle() async {
    ...
    setState(() {
      ...
      _address2.text = _s(selected['addressLine2']);
  ...
  Future<void> _save() async {
    ...
    if (latitudeText.isEmpty != longitudeText.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Latitude and longitude must be supplied together.'),
        ),
      );
      return;
    }
    ...
    if (latitude != null && (latitude < -90 || latitude > 90) ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Latitude or longitude is outside its valid range.'),
        ),
      );
      return;
    }
