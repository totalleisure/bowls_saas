...
class _RinkAssignmentsScreenState extends State<RinkAssignmentsScreen> {
  ...
  // member_profile_id values that are already assigned to any rink slot
  ...
  Future<void> _loadAll() async {
    ...
    try {
      ...
      rinkList.sort((a, b) {
        ...
        final ao = a['fixture_rink_no'] as int;
        final bo = b['fixture_rink_no'] as int;
      ...
      final asnList = List<Map<String, dynamic>>.from(asnRows);
      final assignedSet = asnList
          .map((a) => a['member_profile_id']?.toString())
          .whereType<String>()
          .toSet();
      ...
      for (final a in List<Map<String, dynamic>>.from(asnRows)) {
        ...
        final pos = a['position'] as int;
        byRink.putIfAbsent(rinkId, () => {});
        byRink[rinkId]![pos] = a;
      }

      setState(() {
        _rinks = List<Map<String, dynamic>>.from(rinks);
        _pool = List<Map<String, dynamic>>.from(poolRows);
        _assignedMemberIds = assignedSet;
        _loading = false;
      });
    } catch (e) {
      setState(() {
      ...
      items.add(
        DropdownMenuItem<String?>(
          ...
          child: Builder(builder: (context) {
            final isAssigned = _assignedMemberIds.contains(id);
            return Container(
                  ...
                  children: [
                    if (_rinks.isEmpty)
                      ...
                      ..._rinks.map((r) {
                        ...
                        final order = _asInt(r['fixture_rink_no'], fallback: 0);
                        final fmt = r['format'].toString();
                        final ppr = r['players_per_rink'] as int;
