...
class _ManageTeamScreenState extends State<ManageTeamScreen> {
  ...
  Map<String, dynamic>? _assignmentLocationForMember(String memberId) {
    for (final rinkEntry in _assignmentsByRink.entries) {
      for (final positionEntry in rinkEntry.value.entries) {
        final assignedMemberId = positionEntry.value['member_profile_id']
            ?.toString()
            .trim();

        if (assignedMemberId != memberId) continue;

        final rink = _rinks.cast<Map<String, dynamic>?>().firstWhere(
          (r) => r?['id']?.toString() == rinkEntry.key,
          orElse: () => null,
        );

        final playersPerRink = _asInt(rink?['players_per_rink']);
        final teamNo = rink?['fixture_rink_no']?.toString() ?? '';
        final positionLabel = _positionLabel(
          positionEntry.key,
          playersPerRink,
        );

        return {
          'rink_id': rinkEntry.key,
          'position': positionEntry.key,
          'team_no': teamNo,
  ...
  Future<bool> _confirmMakeReserveRemovesAssignment({
    required String memberId,
    required Map<String, dynamic> assignmentLocation,
  }) async {
    final selectedRow = _selected.firstWhere(
      (r) => r['member_profile_id']?.toString() == memberId,
      orElse: () => <String, dynamic>{},
    ...
    final profile = selectedRow['member_profiles'] as Map<String, dynamic>?;
    final playerName = _displayNameWithPreferredPosition(profile);
    final teamNo = assignmentLocation['team_no']?.toString() ?? '';
    final positionLabel =
        assignmentLocation['position_label']?.toString() ?? 'a team position';

    final locationText = teamNo.isNotEmpty
        ? 'Team $teamNo, $positionLabel'
        : positionLabel;

    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Remove team position?'),
        content: Text(
          playerName.isNotEmpty
              ? '$playerName is already assigned to $locationText. Making them a reserve will remove this team position. Continue?'
              : 'This player is already assigned to $locationText. Making them a reserve will remove this team position. Continue?',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(false),
  ...
  Future<void> _setRole(String memberId, String role) async {
    ...
    final previousAssignments = <String, Map<int, Map<String, dynamic>>>{};
    for (final entry in _assignmentsByRink.entries) {
      previousAssignments[entry.key] = Map<int, Map<String, dynamic>>.from(
        entry.value,
      );
    }
    ...
    try {
      ...
      oldRole = (existing?['role'] ?? '').toString().toLowerCase().trim();
      ...
      final assignmentLocation = newRole == 'reserve'
          ? _assignmentLocationForMember(memberId)
          : null;

      if (assignmentLocation != null) {
        final confirmed = await _confirmMakeReserveRemovesAssignment(
          memberId: memberId,
      ...
      if (assignmentLocation != null) {
        if (mounted) {
          setState(() {
            ...
            _assignmentsByRink.removeWhere(
              (_, byPosition) => byPosition.isEmpty,
            );
          });
        }
      ...
      if (roleUpdated && oldRole.isNotEmpty) {
        try {
          await client
              .from('team_selection_members')
              .update({'role': oldRole})
          ...
          debugPrint('SETROLE rollback error: $rollbackError');
        }
      }

      if (mounted) {
        setState(() {
          _assignmentsByRink = previousAssignments;
